...
| 2026-03-09 | `app/api/env/check/route.ts` | Tool Detection Failure (Python/VS Code) | Code/Python Not Found in UI | Always provide hardcoded path fallbacks for critical tools (C:\Python314, AppData\Local\Programs) as system PATH might not be reliable in some Node.js environments. |
| 2026-03-09 | `lib/automation/ai.ts` | Missing OpenAI Credentials at build | Vercel Build Error (Runtime) | Use lazy initialization (getters) for API clients that depend on environment variables to avoid build-time crashes during module evaluation. |
