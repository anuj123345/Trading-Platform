...
interface HeroProps {
    trustBadge?: {
        text: string;
        icons?: ReactNode[]; // Changed to ReactNode to support lucide-react icons
    };
    headline: {
        line1: string;
        line2: string;
...
const Hero: React.FC<HeroProps> = ({
    ...
    return (
      ...
      `}</style>
            ...
            <canvas
                ref={canvasRef}
                className="absolute inset-0 w-full h-full object-contain touch-none block"
                            ...
                            {trustBadge.icons && (
                                <div className="flex items-center gap-1">
                                    {trustBadge.icons.map((icon, index) => (
                                        <span key={index} className={`text-${index === 0 ? 'yellow' : index === 1 ? 'orange' : 'amber'}-300 flex items-center justify-center h-4 w-4`}>
                                            {icon}
                                        </span>
                                    ))}
                                </div>
